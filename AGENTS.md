...
**Order statuses are fixed team-wide:** `enum :status, { pending: 0, confirmed: 1, delivered: 2, cancelled: 3 }`, default `pending`. `cancelled` covers both an employee cancelling and a provider declining — don't add `rejected`, `completed` or a `canceled` spelling. The frontend gets them as the generated `OrderStatus` type, and `OrderStatusBadge` owns their labels and colors.
